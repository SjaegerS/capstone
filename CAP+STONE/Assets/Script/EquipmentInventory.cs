...
public sealed class EquipmentInventoryRecord
{
    ...
    public bool CanUpgrade => IsOwned && TotalCount >= RequiredCount;

    public EquipmentInventoryRecord(string spriteName, int totalCount, int level, EquipmentCategory category, EquipmentRarityGrade rarity)
    {
...
public static class EquipmentInventory
{
    ...
    private static readonly Dictionary<string, EquipmentInventoryRecord> records = new Dictionary<string, EquipmentInventoryRecord>();
    ...
    public static EquipmentInventoryRecord GetRecord(Sprite sprite)
    {
        if (sprite == null)
        {
            return new EquipmentInventoryRecord(string.Empty, 0, 0, EquipmentCategory.Weapon, EquipmentRarityGrade.Normal);
        }
        ...
        if (!records.TryGetValue(key, out EquipmentInventoryRecord record))
        {
            ...
            EquipmentCategory category = (EquipmentCategory)PlayerPrefs.GetInt(GetCategorySaveKey(sprite.name), (int)GetCategory(sprite));
            EquipmentRarityGrade rarity = (EquipmentRarityGrade)PlayerPrefs.GetInt(GetRaritySaveKey(sprite.name), (int)EquipmentRarityGrade.Normal);
            record = new EquipmentInventoryRecord(sprite.name, count, level, category, rarity);
            records[key] = record;
        }
    ...
    public static EquipmentInventoryRecord GetRecord(string spriteName)
    {
        ...
        if (!records.TryGetValue(key, out EquipmentInventoryRecord record))
        {
            ...
            EquipmentCategory category = (EquipmentCategory)PlayerPrefs.GetInt(GetCategorySaveKey(spriteName), (int)GetCategory(spriteName));
            EquipmentRarityGrade rarity = (EquipmentRarityGrade)PlayerPrefs.GetInt(GetRaritySaveKey(spriteName), (int)EquipmentRarityGrade.Normal);
            record = new EquipmentInventoryRecord(spriteName, count, level, category, rarity);
            records[key] = record;
        }
    ...
    private static void AddToIndex(string spriteName)
    {
        ...
        PlayerPrefs.SetString(IndexSaveKey, string.IsNullOrEmpty(index) ? spriteName : index + "|" + spriteName);
    }
